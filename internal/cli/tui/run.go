// Package tui provides the TUI client CLI logic for `remote-signer tui`.
...
import (
	...
	tuipkg "github.com/ivanzzeth/remote-signer/tui"
)
...
func Run(args []string) error {
	...
	var (
		...
		apiKeyFile        = fs.String("api-key-file", "", "Path to API key PEM file (e.g. data/admin_private.pem); avoids paste")
		apiKeyKeystore    = fs.String("api-key-keystore", "", "Path to Ed25519 encrypted keystore file (mutually exclusive with -api-key-file)")
		apiKeyPasswordEnv = fs.String("api-key-password-env", "", "Environment variable name containing the keystore password (for CI)")
		tlsCA             = fs.String("tls-ca", "", "Path to CA certificate to verify server (for HTTPS)")
		tlsCert           = fs.String("tls-cert", "", "Path to client certificate (for mTLS)")
		tlsKey            = fs.String("tls-key", "", "Path to client private key (for mTLS)")
	...
	if *apiKeyFile != "" && *apiKeyKeystore != "" {
		return fmt.Errorf("-api-key-file and -api-key-keystore are mutually exclusive")
	}
	...
	if *apiKeyID == "" {
		return fmt.Errorf("API key ID is required: use -api-key-id flag or set REMOTE_SIGNER_API_KEY_ID environment variable")
	}
	...
	if *apiKeyKeystore != "" {
		...
		if err != nil {
			return fmt.Errorf("loading key from keystore: %w", err)
		}
		cfg.PrivateKey = privKey
	} else if *apiKeyFile != "" {
		key, err := loadPrivateKeyFromFile(*apiKeyFile)
		if err != nil {
			return fmt.Errorf("loading API key from file: %w", err)
		}
		if isHexKey(key) {
