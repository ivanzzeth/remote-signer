...
gen_go() {
    ...
    # ---------- ② 大小余量 —— 已删除,因为约束没有了 ----------
    #
    # ⭐ 这里曾经有一段检查:client.gen.go 逼近 scripts/arch/40 ⑪b 的 1 MB 上限
    # (2026-09-14 实测 953,958 字节 = 额度的 91%),再加十来个端点就会撞线。
    #
    # 2026-09-15 起产物**不入库**,而 ⑪b 只管**已跟踪文件** —— 所以这条约束对它
    # 不再成立,检查也就没有对象了。⛔ 这不是「放宽了 ⑪b」:⑪b 一个字没改,
    # 它照样拒绝任何超过 1 MB 的已跟踪文件。
    #
    # ⚠️ 顺带说清另一件事,免得后来的人以为拆成两个文件是洁癖:单文件实测
    # 1,050,892 字节,当初拆成 types/client 正是为了绕开那 1 MB。那个规避现在
    # 也不需要了,但两个文件本身有结构意义(类型 / 客户端),所以保留。
