...
| 版本 | Router 类型 | 关键区别 |
| --- | --- | --- |
| V2 | Router02 | 6 个 swap 方法，完整参数级验证：recipient=signer、token path 白名单、金额上限 |
...
V2 验证：router 地址 + 方法选择器白名单 + **完整参数级验证**。所有资金相关参数均做解码校验：
- **recipient (`to`)** 必须等于 signer 地址（防止输出盗取）
- **amountIn / amountInMax** 受 `max_amount_in` 上限约束
- **path[0]**（tokenIn）和 **path[last]**（tokenOut）按白名单校验

5 参数方法（swapExactTokensForTokens、swapTokensForExactTokens、swapExactTokensForETH、swapTokensForExactETH）解码 `(uint256, uint256, address[], address, uint256)`。4 参数方法（swapExactETHForTokens、swapETHForExactTokens）解码 `(uint256, address[], address, uint256)`。两种情况均验证 `to`（recipient）地址等于 signer。

| 规则 | 合约 | 方法 | 选择器 | 参数 | 备注 |
| --- | --- | --- | --- | --- | --- |
| swapExactTokensForTokens | router_address | swapExactTokensForTokens(uint256,uint256,address[],address,uint256) | 0x38ed1739 | recipient=signer<br>amountIn <= max_amount_in<br>path[0] in allowed_token_in<br>path[last] in allowed_token_out | 精确输入，可变输出 |
| swapExactETHForTokens | router_address | swapExactETHForTokens(uint256,address[],address,uint256) | 0x7ff36ab5 | recipient=signer<br>path[0] in allowed_token_in<br>path[last] in allowed_token_out | ETH 输入，token 输出 |
| swapExactTokensForETH | router_address | swapExactTokensForETH(uint256,uint256,address[],address,uint256) | 0x18cbafe5 | recipient=signer<br>amountIn <= max_amount_in<br>path[0] in allowed_token_in<br>path[last] in allowed_token_out | Token 输入，ETH 输出 |
| swapTokensForExactTokens | router_address | swapTokensForExactTokens(uint256,uint256,address[],address,uint256) | 0x8803dbee | recipient=signer<br>amountIn <= max_amount_in<br>path[0] in allowed_token_in<br>path[last] in allowed_token_out | 可变输入，精确输出 |
| swapETHForExactTokens | router_address | swapETHForExactTokens(uint256,address[],address,uint256) | 0xfb3bdb41 | recipient=signer<br>path[0] in allowed_token_in<br>path[last] in allowed_token_out | ETH 输入，精确 token 输出 |
| swapTokensForExactETH | router_address | swapTokensForExactETH(uint256,uint256,address[],address,uint256) | 0x4a25d94a | recipient=signer<br>amountIn <= max_amount_in<br>path[0] in allowed_token_in<br>path[last] in allowed_token_out | Token 输入，精确 ETH 输出 |
...
| allowed_token_in | address_list | 否 | ""（任意） | 逗号分隔的允许输入 token 地址（path[0]） |
| allowed_token_out | address_list | 否 | ""（任意） | 逗号分隔的允许输出 token 地址（path[last]） |
...
V3 验证：router 地址 + 方法选择器白名单 + 单跳 swap 的 tuple 解码 token/金额校验。**recipient 必须等于 signer**（防止输出盗取）。
...
| 规则 | 合约 | 方法 | 选择器 | 参数 | 备注 |
| --- | --- | --- | --- | --- | --- |
| exactInputSingle | router_address | exactInputSingle((address,address,uint24,address,uint256,uint256,uint256,uint160)) | 0x414bf389 | recipient=signer<br>tokenIn 在 allowed_token_in 中<br>tokenOut 在 allowed_token_out 中<br>amountIn <= max_amount_in | Tuple 解码；recipient + 逐 token + 金额验证 |
...
| 规则 | 合约 | 方法 | 选择器 | 参数 | 备注 |
| --- | --- | --- | --- | --- | --- |
| exactOutputSingle | router_address | exactOutputSingle((address,address,uint24,address,uint256,uint256,uint256,uint160)) | 0xdb3e2198 | recipient=signer<br>tokenIn 在 allowed_token_in 中<br>tokenOut 在 allowed_token_out 中<br>amountInMaximum <= max_amount_in | Tuple 解码；recipient + 逐 token + 金额验证 |
