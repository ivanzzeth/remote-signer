...
| S7 requests 闭包 | ✅ 已落地 | |
| S8 rule.go 12 条（⛔ 切 3 个 PR） | ✅ 已落地 | `9b23a8c` |
| S9① spec 从注解生成 + 门禁 ⑭ | ✅ 已落地 | `8a8c7fb` |
| S9② 92 条路由全部注解 | ✅ 已落地 | `90b5e44` |
| S9③ SDK 生成 + 门禁 ⑮（门禁 C） | ✅ 已落地 | `15f6694` |
| （插入）dead-permissions 门禁：定义了却没人读的权限 | ✅ 已落地 | `026b2be` |
| （插入）Go SDK 产物**移出版本库** —— ⛔ 推翻 §4.2 的 Go 那一半 | ✅ 已落地 | `a7dafa9` `089c137` |
| （插入）settings 九组：整份替换 → 合并语义 | ✅ 已落地 | `93711d1` `36c44ff` |
| （插入）模板校验器在自己解析失败时报告「通过」 | ✅ 已修 | |

⛔ **这张表第三次过期了**（2026-09-16 发现）：S8 在 `9b23a8c` 就落了地，S9 三步
在 `8a8c7fb`/`90b5e44`/`15f6694` 全部走完，而表里一直写着「未开始」。⚠️ 它是这条
链路**唯一**的进度记录，写错的代价是下一个人照着它重做一遍已经做完的事。
⭐ 本周一共抓到 **6 处**「当时为真、后来变假且无人对账」的文字（另外五处：
`router.go` 的假权限注释、`large-files.txt` 的余量、`/metrics` 豁免里的
「Recorded, not resolved」、五处 Go SDK 入库理由、`PutSecurity` 的替换语义说明）。
