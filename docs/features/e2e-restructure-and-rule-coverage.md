...
- `cmd/remote-signer-validate-rules/main.go`：若示例/帮助中引用 `rules/polymarket_safe.yaml`，改为引用 `rules/treasury.example.yaml` 或 `config.e2e.yaml`。
...
1. **规则文件清理**  
   ...
   - 更新 `cmd/remote-signer-validate-rules/main.go` 中的示例路径/帮助文案。
