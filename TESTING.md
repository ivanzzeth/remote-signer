...
| `check-openapi.sh` ⑭ 🔒 | The OpenAPI document matches the routes. ⑭a `internal/apidocs/openapi.json` is **regenerated and byte-compared** every run (generation is ~1s, so staleness is measured, not inferred from a timestamp); ⑭b every one of the 104 registered mux patterns is either an operation in that document or a line in [`openapi-routes.txt`](scripts/lib/arch-baseline/openapi-routes.txt) — `todo` (not annotated yet, ratchets to zero) or `exempt` (**cannot** be described, and the reason is mandatory, ≥20 chars: 没有理由的豁免等于关掉检查); ⑭c an operation matching **no** registered route is a hard zero | A spec is the one artefact whose failure mode is silence: it is generated, committed, embedded with `//go:embed` and served by `remote-signer openapi`, so a stale one is a wrong contract handed to every downstream SDK with nothing red anywhere. And the generator has the same shape of self-deception this repo has now hit four times — measured on 2026-09-14: pointed at a directory with no annotations, **swag exits 0 and writes a valid, clean, empty document** (`paths: {}`). A gate that only checked "did generation succeed" would be green while covering zero endpoints. ⑭ therefore counts `@Router` lines in the source tree (independently of swag's own `-d` dirs) and requires that number to equal the operation count; planting the broken `-d` reproduced exactly that red. The route table comes from `cmd/archcheck`'s AST extraction — the same one the four route-auth gates read — **not** from a live router: `NewRouter` needs five dependencies and a database, and `setupRoutes` registers conditionally, so a router-walk describes one deployment (`e2e/test_server.go` leaves 14 `RouterConfig` fields nil and registers no rpc-proxy, broadcast, batch-sign or ACL at all) while looking complete. ⚠️ grep is not a substitute either: `grep 'reg.Handle("'` over today's tree finds **two patterns that do not exist**, quoted in a comment in `module_rules.go` |
| `check-sdk-coverage.sh` ⑮ 🔒 | **Gate C — the end of the chain.** Every operation in `internal/apidocs/openapi.json` is exposed by both generated SDKs, or is a line in [`sdk-routes.txt`](scripts/lib/arch-baseline/sdk-routes.txt) (`todo` ratchets to zero, `exempt` needs a written reason ≥20 chars). ⑮① the generator version is pinned and must match (⚠️ **narrowed 2026-09-15**: neither SDK is committed any more, so "the spec changed and the SDK did not follow" is no longer a reachable state — every run reads artifacts it generated in that same run); ⑮② the operation count read **out of the generated code** must equal the count read out of the spec; ⑮a spec ⊆ SDK; ⑮c an SDK method matching no spec operation is a hard zero; ⑮ **Go side entirely** (`SDK_REGEN=1`, the `sdk-drift` job in `check.yml`, **not** `make check`) — ⛔ the old ⑮d byte-comparison against the committed copy is gone because it lost its subject, not because it was weakened: it guarded against someone hand-editing a generated file, and a file that never enters the repository cannot be hand-edited. ⚠️ `make check` therefore prints "Go SDK 未检查" rather than `Go SDK 0` — a check that did not run must not look like one that ran and passed | ⑭ made "a registered route with no annotation" red. Nothing made **"an operation in the document that no SDK exposes"** red — and that is the last stretch of the chain, the one a caller only discovers at runtime. The generator has the same self-deception as swag, one degree worse because **it compiles**: measured 2026-09-14, `oapi-codegen v2.8.0` on `{"paths":{}}` exits 0 and writes 3,804 bytes of valid Go whose `ClientInterface` is empty — `go build` clean, `go vet` clean, `staticcheck` clean, zero methods. `openapi-typescript 7.13.0` does the same (`export type paths = Record<string, never>`). So ⑮② counts what the generated code itself exposes and requires it to equal the spec's count — two numbers from two files. ⚠️ The extractor's own shape was found by its own assertion, not by reading the templates: proposal §9.2 guessed AST extraction would be brittle; it is not (the path is a literal `fmt.Sprintf` format string, one per operation) but operations **with a body generate two functions**, and the first extractor went red on all 40 of them rather than silently undercounting. ⛔ Why `make check` does not regenerate the Go SDK: oapi-codegen v2.8.0's `go.mod` requires `go >= 1.25.0` while this repo is on 1.24.x, so `go run` switches toolchains — a Go distribution download on a cold runner. A gate that costs a minute gets bypassed (this repo's pre-commit once took 208s and every commit used `--no-verify`), so staleness is proven cheaply here (sha256 + version + counts, ≈1.3s) and byte-equality is proven in a **separate job on the same every-branch workflow** |
