...
| 换钥匙 = **吊销一把、再发一把**,身份不动 | ⛔ 不是「换绑公钥」——那是账户接管原语(PRD **D13**);成员钥匙全丢时产品内不可恢复 |
| 成员可停用而不删除 | 删除会断掉追责链 |
...
- 跨链是**一条策略 × 多条链**(PRD **D15**):差异用**重写**表达 —— 参数不同就重写
  参数,逻辑不同就重写**那一处具名部分**。⛔ 不许整体替换,那是 N 条策略伪装成一条。

**编写入口是 agent**(PRD **D14**):内置条件只服务机器生成,不做拼装界面 ——
旁边已经有一门真语言,再造一门配置拼装的必然长期不一致。

⚠️ 重量因此移到**复核**上:批准的人不再是写的人,N5 从「好用」变**必需**。
...
⭐ **全部已定**(PRD §7)。这一节留着,是因为下一批分叉点会长在这里 —— ⛔ 它们**没有
默认值**,选之前不要往下写实现:实现会替你把它们定死。
